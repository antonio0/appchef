...
class PlayListViewController: UITableViewController, UITableViewDataSource, UITableViewDelegate {
    var appDelegate: AppDelegate?

    var _dataSet: PlayDataSet?
    
    var Elements = PlayElementsCollection()
...
//            btn.addTarget(self, action: "buttonClicked:", forControlEvents: .TouchUpInside)
    ...
    func setDataSet(dataSet: PlayDataSet) {
        _dataSet = dataSet
    ...
    func elTouched(sender: AnyObject?) {
        ...
        var modelToAdd = appDelegate!.playViewController!.DataSets!.getDataSet(modelIdToAdd!)
        modelToAdd!.add(row)
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        var CellElements = PlayElementsCollection(view: cell!.contentView)
