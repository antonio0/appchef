...
class PageViewController: UIViewController {
    ...
    func setId(id: Int) {
        self._id = id
        Elements = PlayElementsCollection(viewController: self)
    }
...
//                castedelement.addTarget!(target: self, action: "navigateToPage:", forControlEvents: UIControlEvents.TouchUpInside)
    ...
    func navigateToPage(sender: UIBarButtonItem) {
        appDelegate!.playViewController!.Pages!.showPage(sender.tag)
    }
    
    func addList(id: Int, source: Int, size: CGRect) {
        
        var newList = appDelegate!.playViewController!.Lists!.create(id, viewController: self, source: source, size: size)
